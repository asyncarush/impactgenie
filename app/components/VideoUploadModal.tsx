...
const VideoUploadModal: React.FC<VideoUploadModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'thumbnail' | 'videoFile') => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      pollIntervalRef.current = setInterval(() => pollProgress(pollIntervalRef.current!), 1000);
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="fixed inset-0 bg-black bg-opacity-50" onClick={!isMinimized ? onClose : undefined} />
        
        <motion.div
          className={`fixed ${isMinimized ? 'bottom-0 left-0' : 'top-1/2 left-1/2'} 
                     ${isMinimized ? 'w-64' : 'w-[90vw] max-w-[560px]'} 
                     transform ${isMinimized ? 'translate-y-0' : '-translate-x-1/2 -translate-y-1/2'}
        ...
        >
          ...
          {!isMinimized && (
            <div className="px-5 py-6">
              <form onSubmit={handleSubmit} className="space-y-5 md:grid md:grid-cols-2 md:gap-5 md:space-y-0">
                {/* Video Upload */}
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-400">Video File*</label>
                  <div className="relative mt-1.5 flex justify-center px-4 py-4 border border-gray-200 dark:border-gray-800 border-dashed rounded hover:border-blue-500/50 dark:hover:border-blue-500/25 bg-gray-50/50 dark:bg-gray-900/25 transition-colors duration-150 group">
                    ...
                    <div className="space-y-3 text-center">
                      <svg className="mx-auto h-9 w-9 text-gray-400 dark:text-gray-600 group-hover:text-blue-500/75 dark:group-hover:text-blue-500/50 transition-colors duration-150" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4v16M17 4v16M3 8h3m-3 4h18m-3-4h3M3 16h3m-3 4h18m-3-4h3M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
                      </svg>
                      <div className="flex flex-col items-center">
                        <span className="text-sm font-medium text-purple-600 dark:text-purple-400 group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-all duration-200">
                          {formData.videoFile ? formData.videoFile.name : 'Upload a video'}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1">MP4, MOV up to 10GB</span>
                        {formData.videoFile && (
                ...
                {/* Thumbnail */}
                <div>
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-400">Thumbnail</label>
                  <div className="relative mt-1.5 flex justify-center px-4 py-4 border border-gray-200 dark:border-gray-800 border-dashed rounded hover:border-blue-500/50 dark:hover:border-blue-500/25 bg-gray-50/50 dark:bg-gray-900/25 transition-colors duration-150 group">
                    <input
                      type="file"
                      accept="image/*"
                    ...
                    <div className="space-y-3 text-center">
                      ...
                      <div className="flex flex-col items-center">
                        <span className="text-sm font-medium text-purple-600 dark:text-purple-400 group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-all duration-200">
                          {formData.thumbnail ? formData.thumbnail.name : 'Upload a thumbnail'}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1">PNG, JPG up to 2MB</span>
                        {formData.thumbnail && (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 mt-2 backdrop-blur-sm">
                            <svg className="-ml-0.5 mr-1.5 h-2 w-2 text-green-400" fill="currentColor" viewBox="0 0 8 8">
                              <circle cx="4" cy="4" r="3" />
