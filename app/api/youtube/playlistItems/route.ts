...
export async function GET(_request: NextRequest) {
  try {
    ...
    const processedVideos = await Promise.all(
      allVideos.map(async (video) => {
        try {
          ...
          console.error(`Error processing video: ${video?.snippet?.resourceId?.videoId}`, err);
          return null;
        }
      })
    );
    ...
    const validVideos = processedVideos.filter((video): video is NonNullable<typeof video> => video !== null);
